...
const _ComboUtils = {
   ...
   *
   * @param {string} selector An exception selector. If the event target matches this selector
   *                          the event will not be killed.
   * @param {Event} event The event object that needs to be killed
   */
  killEventWithException: function (selector, event) {
    if ($(event.target).is(selector)) return;
...
const _OjMultiSelect = _ComboUtils.clazz(_AbstractMultiChoice,
  {
    ...
    _initContainer: function () {
      ...
      // in the dropdown.
      this.dropdown.on('mousedown', _ComboUtils.killEventWithException.bind(null, '.oj-listbox-input'));
    },
