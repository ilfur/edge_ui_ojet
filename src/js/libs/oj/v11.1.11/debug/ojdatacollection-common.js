...
define(['exports', 'jquery', 'ojs/ojcore-base', 'ojs/ojdomutils'], function (exports, $, oj, DomUtils) { 'use strict';
   ...
   * @private
   */
  DataCollectionUtils._ARIA_READONLY = 'aria-readonly';

  /**
   * @private
   */
  DataCollectionUtils._DATA_OJ_ARIA_READONLY_MOD = 'data-oj-ariareadonlymod';
  ...
  DataCollectionUtils.getFocusableElementsInNode = function (node, skipVisibilityCheck) {
    ...
    if (check) {
      ...
      for (var i = 0; i < nodeCount; i++) {
        var elem = nodes[i];
        if (!elem.disabled && (skipVisibilityCheck || DataCollectionUtils.checkVisibility(elem))) {
   ...
   * @private
   */
  DataCollectionUtils.removeAriaReadonly = function (element) {
    var ariaReadonly = element.getAttribute(DataCollectionUtils._ARIA_READONLY);
    // store the aria-readonly value as an attribute
    element.setAttribute(DataCollectionUtils._DATA_OJ_ARIA_READONLY_MOD, ariaReadonly); // @HTMLUpdateOK
    element.removeAttribute(DataCollectionUtils._ARIA_READONLY);
  };
  ...
  DataCollectionUtils.disableAllFocusableElements = function (element, skipVisibilityCheck,
    ...
    for (var i = 0; i < focusElems.length; i++) {
      ...
      if (includeReadonly && focusElems[i].hasAttribute(DataCollectionUtils._ARIA_READONLY)) {
        DataCollectionUtils.removeAriaReadonly(focusElems[i]);
      }
    }
  ...
  DataCollectionUtils.enableAllFocusableElements = function (element) {
    ...
    for (var i = 0; i < focusElems.length; i++) {
      ...
      if (focusElems[i].hasAttribute(DataCollectionUtils._DATA_OJ_ARIA_READONLY_MOD)) {
        var ariaReadonly = focusElems[i].getAttribute(DataCollectionUtils._DATA_OJ_ARIA_READONLY_MOD);
        focusElems[i].removeAttribute(DataCollectionUtils._DATA_OJ_ARIA_READONLY_MOD);
        focusElems[i].setAttribute(DataCollectionUtils._ARIA_READONLY, ariaReadonly); // @HTMLUpdateOK
      }
    }
    return focusElems;
  };
